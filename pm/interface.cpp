        ...
        for(k=dbs;k;k = alpm_list_next(k)){
                ...
                if(match_db(db,sdb)){
                        ...
                        for(i=pkgs;i;i=alpm_list_next(i)){
                                ...
                                if(KMP_cmp(text,name,next,length)||KMP_cmp(text,desc,next,length)){
...
int operation::install_list(alpm_list_t* pkgs)
{
        ...
        alpm_list_t* i,
                *cachedirs = alpm_option_get_cachedirs(conf->handle);
        if(cachedirs){
        ...
        while(!pkg_queue.empty()){
                ...
                for(i=depend;i;i=alpm_list_next(i)){
                        ...
                        if(!get_localpkg(name)) {
                                if(dsu_find(far,hash_compute(name)%dsu_len)==dsu_find(far,pkg,dsu_len)){
                                        continue;
                                }
                                alpm_list_t* dbs = alpm_get_syncdbs(conf->handle);
...
int operation::match_db(alpm_db_t* db, int sdb)
{
        ...
        else if((sdb & COMMUNITY) && strcmp(alpm_db_get_name(db),"community")==0 ){
                return COMMUNITY;
        }
...
bool operation::check_higher_version(alpm_pkg_t* pkg)
{
        ...
        for(i=pkgcache;i;i=alpm_list_next(i)){
                alpm_pkg_t* p = (alpm_pkg_t*)i->data;
                if(cmp_pkg(p,pkg)){
                        return alpm_pkg_vercmp(alpm_pkg_get_version(pkg),alpm_pkg_get_version(p))<=0;
                }
        }
        return false;
}
...
static trie_node* list_find_char(alpm_list_t* list,char t)
{
        ...
        for(i=list;i;i=alpm_list_next(i)){
                ...
                if(node->type==2&&node->alpha==t)return node;
        }
        return nullptr;
}
...
void operation::dsu_release(int* dsu)
{
        free(dsu);
}
...
void operation::dsu_make(int* dsu,pkg_hash h1,pkg_hash h2)
{
        dsu[h1] = h2;
}
