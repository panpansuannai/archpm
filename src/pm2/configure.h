...
static void set_up_callback_func() { return; }
...
public:
  ...
  using ConfigurePtr = std::shared_ptr<Configure>;
  Configure(string root, string dbpath)
      : handle_(nullptr), configure_file_(DEF_CONFIG_FILE) {
    alpm_errno_t err;
    handle_ = alpm_initialize(root.c_str(), dbpath.c_str(), &err);
    if (!handle_) {
      LOG << "[E] Configure: Initialize handle err :"
  ...
  ~Configure() { alpm_release(handle_); }

  bool setup_opt(OptionPtr opt) { return opt->do_opt(handle_); }

  string get_database_path() { return string(alpm_option_get_dbpath(handle_)); }
  string get_log_file() { return string(alpm_option_get_logfile(handle_)); }
  string get_architecture() { return string(alpm_option_get_arch(handle_)); }

  string get_gpg_directory() { return string(alpm_option_get_gpgdir(handle_)); }
